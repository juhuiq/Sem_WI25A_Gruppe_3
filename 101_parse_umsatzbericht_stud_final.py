...
def xfile_read(wdir, in_file, tabble_name):
    ...
    # Ihr Code folgt hier: ...
    df = xl.parse(tabble_name) # lesen von inhalten in der excel-tabelle
    if DEBUG_INFO: print(f"df: {df}")   #ausgabe des dataframes


    # printing the header row as list of values, e.g.
    # header_row:  ['Produkt', 'Kunde', 'Qrtl 1', 'Qrtl 2', 'Qrtl 3', 'Qrtl 4']
    header_row = df.columns.tolist() #header row neue variable  die spalten namen in liste speichert
    # printing the number of values in the header row, e.g. wird schon in while schleife gemacht


    number_of_headers = len(header_row)#speichert Anzahl an spalten von header_row
...
def plot_client_data(df, kunde_produkt, quartal):
    ...
    # Ihr Code folgt hier: ...
    x_axis_raw = df[kunde_produkt].tolist() #liste mit allen kunden oder produkten
    ...
    # Ihr Code folgt hier: ...
    x_axis_clean = sorted(list(set(x_axis_raw))) #set = ansammlung ohne Dopplungen sortiert nach alphabet
    ...
    y_axis = {} #y-achse erstellen
    for x_axis_value in x_axis_clean:
        y_axis[x_axis_value] = 0 #jeder y wert auf Standardwert 0 setzen
    ...
    # Ihr Code folgt hier: ...
    for x_axis_value in x_axis_raw: #für jeden kunden oder produkt
        if quartal == "all": #wenn alle quartale ausgewählt sind
            for q in ["Qrtl 1", "Qrtl 2", "Qrtl 3", "Qrtl 4"]: #für jedes quartal
                y_axis[x_axis_value] += df.loc[df[kunde_produkt] == x_axis_value, q].sum() #summe der quartale für jeden kunden oder produkt
        else: #wenn nur ein quartal ausgewählt ist
            y_axis[x_axis_value] += df.loc[df[kunde_produkt] == x_axis_value, quartal].sum() #summe des ausgewählten quartals für jeden kunden oder produkt
#df.loc gibt liste die die quatalwerte beinhaltet nur von der einen Person oder produkt und addiert alle werte in liste
# durch for schleife gehts zum nächsten produkt oder kunde  
    ...
    gesamtumsatz = sum(y_axis.values())#gesamtumsatz berechnen
    if gesamtumsatz > 0:#wenn gesamtumsatz größer als 0
        prozentuale_anteile = {x: f"{v / gesamtumsatz * 100:.2f}%" for x, v in y_axis.items()}# prozentuale anteile berechnen

    else:
        prozentuale_anteile = {k: 0 for k in y_axis.keys()}
    print(f"\nProzentualer Anteil der Umsätze am Gesamtumsatz über das Quartal '{quartal}' für '{kunde_produkt}' ...\n{prozentuale_anteile}")
    ...
    # Ihr Code folgt hier: ...
    plt.bar(x_axis_clean, [y_axis[x] for x in x_axis_clean])# diagramm wird geplottet
    plt.title(f"Umsätze über das Quartal '{quartal}' für '{kunde_produkt}'")
   # 1. Durchschnitt berechnen
    anzahl_werte = len(y_axis)
    if anzahl_werte > 0:
        durchschnitt = gesamtumsatz / anzahl_werte
    else:
        durchschnitt = 0

    # 2. Waagerechte Linie zeichnen
    # color='r' (rot), linestyle='--' (gestrichelt)
    plt.axhline(y=durchschnitt, color='r', linestyle='--', label=f'Ø Umsatz: {durchschnitt:.2f}')
    
    # 3. Legende aktivieren (damit die Beschriftung der Linie angezeigt wird)
    plt.legend()
...
if __name__ == "__main__":
    ...
    wdir = ''
    infile = '101_Umsatzbericht.xlsx'
    table_name = 'Quelldaten'
    try:
        df, header_row, number_of_headers = xfile_read(wdir, infile, table_name)
    except Exception as e:
        print(e)
        print(f"Sorry. Could not parse input data correctly. Did you complete your work inside xfile_read()? Error: {e}")
        raise SystemExit
    ...
    while not parsing_completed:
        ...
        if parsing_completed:
            plot_client_data(df, kunde_produkt, quartal)# funktionsaufruf
        else:
